...
public class BookcodeDao {
	...
	public void deleteBookcode(Connection conn, int bookcodeNo) {
		//초기값.
		PreparedStatement pstmtdeleteBookcode = null;
		
		//쿼리문을 String data type 변수인 sqldeldeteBookcode에 대입.
		String sqldeleteBookcode = "DELETE FORM bookcode where bookcode_no=?";
		...
		try {
			...
			//쿼리 실행및, 삭제된 레코드 수 출력.
			System.out.println("삭제된 bookcode 레코드의 수 :" + pstmtdeleteBookcode.executeLargeUpdate());
		...
		//객체 종료
			...
			if(pstmtdeleteBookcode != null) {
				try {
					pstmtdeleteBookcode.close();
				} catch(SQLException e) {
					System.out.println("DB와 관련된 예외가 발생하였습니다, deleteBookcode close");
					e.printStackTrace();
				}
			}
		}
